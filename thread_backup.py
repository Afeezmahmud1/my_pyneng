...
if __name__ == "__main__":
    config_command_dict = {'10.2.2.11':['vlan 11','name VLAN-11'],'10.2.2.13':['ip access-list extended DEVNET','permit ip any any']}
    config_command_list = ['router ospf 100','router-id 11.11.11.11','network 0.0.0.0 0.0.0.0 area 0']
    config_command_str = 'ip http server'
    show_command_str = 'show clock'
    show_command_list = ["sh clock","sh version | in uptime"]
    show_command_dict = {"10.2.2.11": "sh ip int br","10.2.2.13": "sh cdp nei"}
    out_file = 'netmiko_1_thread.txt'
    with open ('/root/Desktop/PYNENG-FOLDER/devices.yaml') as f:
        devices = yaml.safe_load(f)
    thread_command(devices,out_file,show=show_command_str)    
